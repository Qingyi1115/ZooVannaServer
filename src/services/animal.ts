...
export async function getAnimalObservationLogById(
  ...
  if (!animalObservationLog) throw {message:"Unable to find AnimalObservationLog with Id " + animalObservationLogId}
  return animalObservationLog;
}
...
export async function getAnimalObservationLogsBySpeciesCode(
  ...
  const species = await SpeciesService.getSpeciesByCode(speciesCode, ["animals"]);
...
export async function updateAnimalObservationLog(
  ...
  const animalObservationLog = await getAnimalObservationLogById(animalObservationLogId);
...
export async function deleteAnimalObservationLogById(
  ...
  const animalObservationLog = await getAnimalObservationLogById(animalObservationLogsId);
