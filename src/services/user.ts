...
export async function employeeLogin(employeeEmail:string, password:string):Promise<boolean> {
    return !!(await Employee.findOne({where:{employeeEmail:employeeEmail}}))?.testPassword(password);
}